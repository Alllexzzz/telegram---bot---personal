...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Здарова, братан! Я ассистент на DeepSeek.\n\n"
        "Команды:\n/ideas — все идеи\n/reminders — активные напоминания\n"
        "Запомнить: 'запомни: твоя идея'\n"
        "Напомнить: 'напомни через 30 минут сделать что-то'\n"
...
async def reminders_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not reminders:
        ...
        for r in reminders:
            ...
            if rt > now:
                msg += f"• {r['text']} (в {rt.strftime('%d.%m.%Y %H:%M')})\n"
        await update.message.reply_text(msg)
...
async def clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    memory = []
    save_memory()
    await update.message.reply_text("Всё забыл. Контекст очищен.")
...
def main():
    ...
    logger.info("Братван-бот на DeepSeek запущен!")
    app.run_polling()
